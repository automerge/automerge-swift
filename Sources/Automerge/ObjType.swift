...
public enum ObjType: Sendable {
    ...
    ///
    /// A `map` type can be used to represent a Swift dictionary, or an swift value or reference type encoded in the document using ``AutomergeEncoder``.
    /// An Automerge `map` type uses `String` for keys, and can reference any other Automerge object type or a ``ScalarValue``.
    ...
    ///
    /// This type is frequently correlated with the ``AutomergeText`` class, which provides a convenient read/write interface to the encapsulated String.
