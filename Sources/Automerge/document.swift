...
public class Document {
  ...
  public func insertObject(obj: ObjId, index: UInt64, ty: ObjType) throws -> ObjId {
    try self.doc.wrapErrors {
      ObjId(bytes: try $0.insertObjectInList(obj: obj.bytes, index: index, objType: ty.toFfi()))
    }
  }
    ...
    -> Value?
  {
    let val = try self.doc.wrapErrors {
      try $0.getAtInMap(obj: obj.bytes, key: key, heads: heads.map({ $0.bytes }))
    }
    return val.map(Value.fromFfi)
  }
  ...
  {
    let val = try self.doc.wrapErrors {
      try $0.getAtInList(obj: obj.bytes, index: index, heads: heads.map({ $0.bytes }))
    }
    return val.map(Value.fromFfi)
  }
    ...
    -> Set<Value>
  {
    let vals = try self.doc.wrapErrors {
      try $0.getAllAtInMap(obj: obj.bytes, key: key, heads: heads.map({ $0.bytes }))
    }
    return Set(vals.map({ Value.fromFfi(value: $0) }))
  }
  ...
  {
    let vals = try self.doc.wrapErrors {
      try $0.getAllAtInList(obj: obj.bytes, index: index, heads: heads.map({ $0.bytes }))
    }
    return Set(vals.map({ Value.fromFfi(value: $0) }))
  }
  ...
  public func keysAt<Heads: Collection<ChangeHash>>(obj: ObjId, heads: Heads) -> [String] {
    self.doc.wrapErrors { $0.mapKeysAt(obj: obj.bytes, heads: heads.map({ $0.bytes })) }
  }
  ...
  public func valuesAt<Heads: Collection<ChangeHash>>(obj: ObjId, heads: Heads) throws -> [Value] {
    let vals = try self.doc.wrapErrors {
      try $0.valuesAt(obj: obj.bytes, heads: heads.map({ $0.bytes }))
    }
    return vals.map({ Value.fromFfi(value: $0) })
  }
  ...
  public func mapEntriesAt<Heads: Collection<ChangeHash>>(obj: ObjId, heads: Heads) throws -> [(
    ...
    let entries = try self.doc.wrapErrors {
      try $0.mapEntriesAt(obj: obj.bytes, heads: heads.map({ $0.bytes }))
    }
    return entries.map({ ($0.key, Value.fromFfi(value: $0.value)) })
  }
  ...
  public func lengthAt<Heads: Collection<ChangeHash>>(obj: ObjId, heads: Heads) -> UInt64 {
    self.doc.wrapErrors { $0.lengthAt(obj: obj.bytes, heads: heads.map({ $0.bytes })) }
  }
  ...
  public func textAt<Heads: Collection<ChangeHash>>(obj: ObjId, heads: Heads) throws -> String {
    try self.doc.wrapErrors { try $0.textAt(obj: obj.bytes, heads: heads.map({ $0.bytes })) }
  }
  ...
  public func fork() -> Document {
    return Document(doc: self.doc.wrapErrors { $0.fork() })
  }
  ...
  public func forkAt<Heads: Collection<ChangeHash>>(heads: Heads) throws -> Document {
    return try self.doc.wrapErrors { Document(doc: try $0.forkAt(heads: heads.map({ $0.bytes }))) }
  }
  ...
  public func heads() -> Set<ChangeHash> {
    return Set(self.doc.wrapErrors { $0.heads().map({ ChangeHash(bytes: $0) }) })
  }
  ...
  public func encodeNewChanges() -> [UInt8] {
    return self.doc.wrapErrors { $0.encodeNewChanges() }
  }
  ...
  public func encodeChangesSince<Heads: Collection<ChangeHash>>(heads: Heads) throws -> [UInt8] {
    return try self.doc.wrapErrors { try $0.encodeChangesSince(heads: heads.map({ $0.bytes })) }
  }
